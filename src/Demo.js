import * as NodeGraph from "@/NodeGraph.js";
...
function demo1() {
	const perlin1 = NodeGraph.addNode("Perlin Noise", 50, 50);
	Node.setFinalPropertyValueAndPropagate(perlin1, "scale", 25);

	const perlin2 = NodeGraph.addNode("Perlin Noise", 50, 450);
	Node.setFinalPropertyValueAndPropagate(perlin2, "seed", 10);

	const blend1 = NodeGraph.addNode("Blend", 400, 250);
	Node.setFinalPropertyValueAndPropagate(blend1, "mode", "Multiply");

	const output = NodeGraph.addNode("Output", 750, 250);
...
function demo3() {
	// Perlin Noise
	const perlin1 = NodeGraph.addNode("Perlin Noise", -200, 0);
	Node.setFinalPropertyValueAndPropagate(perlin1, "scale", 10);

	const perlin2 = NodeGraph.addNode("Perlin Noise", -200, 600);
	Node.setFinalPropertyValueAndPropagate(perlin2, "scale", 20);
	Node.setFinalPropertyValueAndPropagate(perlin2, "seed", 1);

	const perlin3 = NodeGraph.addNode("Perlin Noise", 200, 400);
	Node.setFinalPropertyValueAndPropagate(perlin3, "scale", 5);
	Node.setFinalPropertyValueAndPropagate(perlin3, "seed", 2);

	// Blend
	const blend1 = NodeGraph.addNode("Blend", 200, 0);
	Node.setFinalPropertyValueAndPropagate(blend1, "mode", "Multiply");

	const blend2 = NodeGraph.addNode("Blend", 600, 0);
	Node.setFinalPropertyValueAndPropagate(blend2, "mode", "Overlay");

	const blend3 = NodeGraph.addNode("Blend", 1000, 400);

	const blend4 = NodeGraph.addNode("Blend", 1400, 600);
	Node.setFinalPropertyValueAndPropagate(blend4, "mode", "Dissolve");

	const blend5 = NodeGraph.addNode("Blend", 1800, 400);
	Node.setFinalPropertyValueAndPropagate(blend5, "opacity", 0.85);

	// Output
	const output = NodeGraph.addNode("Output", 1800, 200);
