...
// Modified from https://github.com/ashima/webgl-noise/blob/master/src/cellular2D.glsl
...
vec2 cellular(vec2 P) {
	...
	vec3 dx = Pf.x + 0.5 + u_jitter*ox;
	vec3 dy = Pf.y - of + u_jitter*oy;

	vec3 d1 = dx * dx + dy * dy; // d11, d12 and d13, squared
	p = permute(px.y + Pi.y + oi); // p21, p22, p23
	ox = fract(p*K) - Ko;
	oy = mod7(floor(p*K))*K - Ko;
	dx = Pf.x - 0.5 + u_jitter*ox;
	dy = Pf.y - of + u_jitter*oy;

	vec3 d2 = dx * dx + dy * dy; // d21, d22 and d23, squared
	p = permute(px.z + Pi.y + oi); // p31, p32, p33
	ox = fract(p*K) - Ko;
	oy = mod7(floor(p*K))*K - Ko;
	dx = Pf.x - 1.5 + u_jitter*ox;
	dy = Pf.y - of + u_jitter*oy;
